...
if [[ -n $files ]] ; then
    ...
    if ! Rscript --vanilla -e 'library(littler)' &> /dev/null ; then
        echo "Commit aborted: please install the littler package in R." >&2
        exit 1
    fi
fi

for file in $files ; do
    Rscript --vanilla .git/hooks/git-hook-lintr.R $file || exit 1
done